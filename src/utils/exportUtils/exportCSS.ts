...
function mutateCssText(css: string): string {
    // replace used fonts so that we don't have to bundle Vazirmatn & Inconsalata
    const sansFont = `"Vazirmatn", -apple-system, BlinkMacSystemFont, avenir next,
            avenir, segoe ui, helvetica neue, helvetica, Ubuntu, roboto, noto, arial, sans-serif`;
    return css
        .replace(/font-family: ?(Vazirmatn|'Vazirmatn'|"Vazirmatn")/g, `font-family: ${sansFont}`)
        .replace(/--cpd-font-family-sans: ?(Vazirmatn|'Vazirmatn'|"Vazirmatn")/g, `--cpd-font-family-sans: ${sansFont}`)
        .replace(
